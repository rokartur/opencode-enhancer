...
function mergeSettings(base: RotationSettings, overrides?: Partial<RotationSettings>): RotationSettings {
	return {
		...base,
		...overrides,
		accountWeights: overrides?.accountWeights ? { ...overrides.accountWeights } : { ...base.accountWeights },
		featureFlags: {
...
function resolveSettings(includeEnvOverrides: boolean): SettingsResult {
	...
	if (includeEnvOverrides) {
		...
		if (
			envStrategy &&
			['round-robin', 'least-used', 'random', 'weighted-round-robin', 'usage-priority'].includes(envStrategy)
		) {
		...
		const envAutoSwitch = readEnv('OPENCODE_ENHANCER_AUTOSWITCH_ENABLED', 'OPENCODE_MULTI_AUTH_AUTOSWITCH_ENABLED')
		if (envAutoSwitch !== undefined) {
			const enabled = envAutoSwitch.toLowerCase() === 'true' || envAutoSwitch === '1'
			settings.featureFlags = {
				...(settings.featureFlags || DEFAULT_FEATURE_FLAGS),
				autoSwitch: enabled,
			}
		...
		if (envNotifyWhenTerminalActive !== undefined) {
			const enabled = envNotifyWhenTerminalActive.toLowerCase() === 'true' || envNotifyWhenTerminalActive === '1'
...
// Runtime behavior should use persisted settings so local config changes take effect immediately.
