...
function resolveSettings(includeEnvOverrides: boolean): SettingsResult {
  ...
  if (includeEnvOverrides) {
    ...
    if (
      envStrategy &&
      ["round-robin", "least-used", "random", "weighted-round-robin", "usage-priority"].includes(
        envStrategy,
      )
    ) {
    ...
    const envAutoSwitch = readEnv(
      "OPENCODE_ENHANCER_AUTOSWITCH_ENABLED",
      "OPENCODE_MULTI_AUTH_AUTOSWITCH_ENABLED",
    );
