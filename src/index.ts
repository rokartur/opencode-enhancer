...
function buildAccountSelectOption(account: AccountCredentials): { label: string; value: string; hint: string } {
...
const MultiAuthPlugin: Plugin = async ({ client, $, serverUrl, project, directory }: PluginInput) => {
  ...
  return Number.isFinite(parsed) && parsed > 0 ? parsed : TIMEOUTS.UPSTREAM_FETCH_MS
              ...
              return new Response(
                JSON.stringify({ error: { code: 'REQUEST_FAILED', message: `[enhancer] Request failed: ${err}` } }),
                  ...
                  return {
                    ...
                    callback: async () => {
                      try {
                        ...
                        return { type: 'success' as const, provider: PROVIDER_ID, refresh: acc.refreshToken, access: acc.accessToken, expires: acc.expiresAt }
                      } catch {
                        return { type: 'failed' as const }
                      }
                    }
