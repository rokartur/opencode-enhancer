...
function resolveUpstreamTimeoutMs(isStreaming: boolean): number {
	...
	return (
		parsePositiveEnvNumber('OPENCODE_ENHANCER_UPSTREAM_TIMEOUT_MS', 'OPENCODE_MULTI_AUTH_UPSTREAM_TIMEOUT_MS') ||
...
function formatUsageWindow(label: string, window: AccountRateLimits['fiveHour']): string | undefined {
...
function formatAccountUsageSummary(rateLimits?: AccountRateLimits): string {
	const parts = [formatUsageWindow('5h', rateLimits?.fiveHour), formatUsageWindow('wk', rateLimits?.weekly)].filter(
		Boolean,
	)
	return parts.join(' · ')
}
...
function normalizeModel(model: string | undefined): string {
	...
	if (
		...
		const latestModel = (
			readEnv('OPENCODE_ENHANCER_CODEX_LATEST_MODEL', 'OPENCODE_MULTI_AUTH_CODEX_LATEST_MODEL') || 'gpt-5.4'
		).trim()
...
const MultiAuthPlugin: Plugin = async ({ client, $, serverUrl, project, directory }: PluginInput) => {
	...
	const TERMINAL_BUNDLE_IDS: Record<NonNullable<TerminalNotificationSupport['terminal']>, string[]> = {
	...
	const shouldMirrorTerminalNotificationToSystem = (terminalSupport: TerminalNotificationSupport): boolean => {
	...
	const escapeAppleScriptString = (value: string): string => {
		return String(value).replaceAll('\\', '\\\\').replaceAll('"', '"').replaceAll(String.fromCharCode(10), '\n')
	}
	...
	const formatContextBody = async (sessionID: string | undefined, lines: string[]): Promise<string> => {
	...
	const notifyNtfy = async (title: string, body: string, priority: string, clickUrl?: string): Promise<void> => {
	...
	const notifyTargets = async (title: string, body: string, priority: string, clickUrl?: string): Promise<void> => {
	...
	return {
		...
		'tool.execute.before': async input => {
			...
			const body = await formatContextBody(input.sessionID || undefined, ['Question requires your input'])
			await notifyTargets(
				formatTitle('question'),
				body,
				'4',
		...
		config: async config => {
			...
			try {
				...
				if ((latestModel === 'gpt-5.5' || latestModel === 'gpt-5.4') && defaultModels[`${latestModel}-fast`]) {
				...
				const customFetch = async (input: Request | string | URL, init?: RequestInit): Promise<Response> => {
					...
					while (attempt < maxAttempts) {
									...
									return new Response(
										JSON.stringify({
											error: {
												...
												details: {
													alias: forced.alias,
													rateLimitedUntil: forced.rateLimitedUntil,
												},
											},
										}),
						...
						if (supportedFastMode) {
							...
							if (isDebugEnabled()) {
								console.log(`[enhancer] fast mode enabled: ${normalizedModel} + service_tier=priority`)
							}
							...
							authorize: async (inputs?: Record<string, string>) => {
								...
								return {
									...
									callback: async () => {
										...
										debugLog(`token refresh failed for ${account.alias}, attempting silent re-auth`)
										try {
