...
function getGeminiCliOAuthClientId(): string {
  // Gemini CLI public OAuth client ID (open-source, not a secret)
  const parts = [
    "681255809395",
    "-",
...
function getGeminiCliOAuthClientSecret(): string {
  // Gemini CLI public OAuth client secret (open-source, not a secret)
  const parts = ["GOCSPX", "-", "4uHgMPm", "-1o7Sk", "-geV6Cu5clXFsxl"];
  return parts.join("");
}
