...
export const codexProvider: UsageProvider = {
  ...
  async fetchUsage(): Promise<ProviderResult> {
      ...
      accounts.map(async (acc): Promise<ProviderAccountResult> => {
        ...
        try {
          ...
          if (fh?.remaining !== undefined) {
            ...
            windows.push({
              utilization: fhLimit === 100 ? Math.max(0, 100 - fh.remaining) : Math.max(0, Math.round(((fhLimit - fh.remaining) / fhLimit) * 100)),
              resetsAt: fh.resetAt,
              label: '5h',
            })
          }
          if (wk?.remaining !== undefined) {
            ...
            windows.push({
              utilization: wkLimit === 100 ? Math.max(0, 100 - wk.remaining) : Math.max(0, Math.round(((wkLimit - wk.remaining) / wkLimit) * 100)),
              resetsAt: wk.resetAt,
              label: 'weekly',
            })
          }
          ...
          const fhUtil = fh?.remaining !== undefined
            ? (fhLimit === 100 ? 100 - fh.remaining : ((fhLimit - fh.remaining) / fhLimit) * 100)
            : 0
          const wkUtil = wk?.remaining !== undefined
            ? (wkLimit === 100 ? 100 - wk.remaining : ((wkLimit - wk.remaining) / wkLimit) * 100)
            : 0
    ...
    const accountResults: ProviderAccountResult[] = results.map((r) =>
      r.status === 'fulfilled'
        ? r.value
        : {
            label: '?',
            ...
            error: `${r.reason}`,
            usage: { type: 'quotaBased' as const, utilization: 0, windows: [] },
          }
    )
    ...
    if (okAccounts.length > 0) {
      const best = okAccounts.reduce((a, b) =>
        a.usage.utilization < b.usage.utilization ? a : b
      )
    ...
    return {
      ...
      error: anyOk ? undefined : accountResults.map((a) => a.error).filter(Boolean).join('; '),
      fetchedAt: Date.now(),
    }
  },
}
