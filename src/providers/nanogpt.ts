...
interface NanoGptUsageResponse {
	weeklyInputTokens?: TokenWindow
	dailyInputTokens?: TokenWindow
	monthlyInputTokens?: TokenWindow
	limits?: {
...
export const nanoGptProvider: UsageProvider = {
	id: 'nanogpt',
	name: 'Nano-GPT',
	...
	async fetchUsage(): Promise<ProviderResult> {
		...
		try {
			...
			if (usageRes.status === 'fulfilled' && usageRes.value.ok) {
				const usage = (await usageRes.value.json()) as NanoGptUsageResponse
				...
				const errText = usageRes.status === 'fulfilled' ? `HTTP ${usageRes.value.status}` : `${usageRes.reason}`
			...
			if (balanceRes.status === 'fulfilled' && balanceRes.value.ok) {
				const balance = (await balanceRes.value.json()) as NanoGptBalanceResponse
			...
			return {
				...
				usage: windows.length > 0 ? { type: 'quotaBased', utilization: maxUtilization, windows } : undefined,
