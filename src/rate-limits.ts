...
export function hasMeaningfulRateLimits(rateLimits: AccountRateLimits | undefined | null): boolean {
	...
	return hasMeaningfulRateLimitWindow(rateLimits.fiveHour) || hasMeaningfulRateLimitWindow(rateLimits.weekly)
}
...
export function mergeRateLimits(existing: AccountRateLimits | undefined, update: RateLimitUpdate): AccountRateLimits {
...
export function parseRetryAfterHeader(retryAfter: string | null, now: number = Date.now()): number | undefined {
...
export function parseRateLimitResetFromError(text: string, now: number = Date.now()): number | undefined {
	...
	if (retryAfterMatch) {
		...
		if (Number.isFinite(amount) && amount >= 0) {
			if (unit.startsWith('h')) return now + amount * 60 * 60 * 1000
			if (unit.startsWith('m')) return now + amount * 60 * 1000
