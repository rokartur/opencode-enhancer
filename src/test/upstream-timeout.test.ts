...
async function createCustomFetch(): Promise<(input: Request | string | URL, init?: RequestInit) => Promise<Response>> {
	const { default: MultiAuthPlugin } = await importFreshModule<typeof import('../index.js')>('../index.js')
...
test('non-stream timeout returns 504 with deterministic REQUEST_TIMEOUT error', async () => {
	await withTempStore(async () => {
		...
		try {
			const response = await customFetch('https://api.openai.com/v1/responses', {
				method: 'POST',
				body: JSON.stringify({ model: 'gpt-5.5', stream: false }),
			})
