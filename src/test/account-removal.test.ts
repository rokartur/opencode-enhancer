...
import { addAccount, getStorePath, invalidateStoreCache, loadStore, removeAccount, updateAccount } from '../store.js'
...
test('syncCodexAuthFile does not recreate a removed account', async () => {
	await withTempStore(async () => {
		await withTempCodexAuthFile(async authFile => {
			...
			const { syncCodexAuthFile } = await importFreshModule<typeof import('../codex-auth.js')>('../codex-auth.js')
