...
const ROTATION_STRATEGIES = new Set(['round-robin', 'least-used', 'random', 'weighted-round-robin', 'usage-priority'])
...
export function activateForce(
	...
	const keepExistingTtl =
		store.forcedAlias === alias && typeof store.forcedUntil === 'number' && store.forcedUntil > now
	const forcedUntil = keepExistingTtl ? store.forcedUntil! : now + FORCE_TTL_MS

	const currentStrategy = store.settings?.rotationStrategy || store.rotationStrategy || 'usage-priority'
