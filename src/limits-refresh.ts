...
export async function refreshRateLimitsForAccount(account: AccountCredentials): Promise<LimitRefreshResult> {
	...
	if (usage.error) {
		if (usage.shouldProbeFallback === false) {
			...
			if (usage.workspaceDeactivated) {
				markWorkspaceDeactivated(account.alias, DEFAULT_CONFIG.workspaceDeactivatedCooldownMs, {
	...
	if (!probe.isAuthoritative || !probe.rateLimits) {
		...
		const fallbackResetAt = likelyRateLimit ? getBlockingRateLimitResetAt(account.rateLimits, now) : undefined
...
export async function refreshRateLimits(accounts: AccountCredentials[], alias?: string): Promise<LimitRefreshResult[]> {
	if (alias) {
