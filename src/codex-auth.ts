...
function getCodexAuthFilePath(): string {
  const override = process.env[CODEX_AUTH_FILE_ENV] || process.env[LEGACY_CODEX_AUTH_FILE_ENV];
...
function ensureDir(): void {
  const dir = path.dirname(CODEX_AUTH_FILE);
...
export function loadCodexAuthFile(): CodexAuthFile | null {
  ...
  try {
    ...
    console.error("[enhancer] Failed to parse codex auth.json:", err);
    return null;
  }
}
...
export function getNameFromClaims(claims: Record<string, any> | null): string | undefined {
  if (!claims) return undefined;
  ...
  const profile = claims["https://api.openai.com/profile"] as { name?: string } | undefined;
...
export function getAccountUserIdFromClaims(claims: Record<string, any> | null): string | undefined {
  if (!claims) return undefined;
...
export function getUserIdFromClaims(claims: Record<string, any> | null): string | undefined {
  if (!claims) return undefined;
  const auth = claims["https://api.openai.com/auth"] as
    | { user_id?: string; chatgpt_user_id?: string }
    | undefined;
...
export function syncCodexAuthFile(options?: { setActiveAlias?: boolean; allowAdd?: boolean }): {
  alias: string | null;
  added: boolean;
  updated: boolean;
  authEmail?: string;
  ...
  const idClaims = normalized.idToken ? decodeJwtPayload(normalized.idToken) : null;
  const email = getEmailFromClaims(idClaims) || getEmailFromClaims(accessClaims);
  const name = getNameFromClaims(idClaims) || getNameFromClaims(accessClaims);
  const accountId =
    normalized.accountId ||
    getAccountIdFromClaims(idClaims) ||
    getAccountIdFromClaims(accessClaims);
  ...
  const planType = getPlanTypeFromClaims(accessClaims) || getPlanTypeFromClaims(idClaims);
  const expiresAt =
    getExpiryFromClaims(accessClaims) || getExpiryFromClaims(idClaims) || Date.now();
  ...
  if (isRemovedAccount({ accountId, accountUserId, userId, email })) {
    return {
      alias: null,
      added: false,
      updated: false,
