...
function buildGeneratedAlias(options?: { preferredAlias?: string; email?: string }): string {
	...
	return new Promise((resolve, reject) => {
		...
		server = http.createServer(async (req, res) => {
			...
			try {
				...
				const expiresAt =
					getExpiryFromClaims(accessClaims) || getExpiryFromClaims(idClaims) || now + tokens.expires_in * 1000

				let email: string | undefined = getEmailFromClaims(idClaims) || getEmailFromClaims(accessClaims)
				let name: string | undefined = getNameFromClaims(idClaims) || getNameFromClaims(accessClaims)
				try {
...
export async function refreshToken(alias: string): Promise<AccountCredentials | null> {
	...
	try {
		...
		const idClaims = tokens.id_token ? decodeJwtPayload(tokens.id_token) : null
		const expiresAt =
			getExpiryFromClaims(accessClaims) || getExpiryFromClaims(idClaims) || Date.now() + tokens.expires_in * 1000
