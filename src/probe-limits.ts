...
function getAuthClaim(claims: Record<string, any> | null): Record<string, any> | undefined {
	if (!claims) return undefined
...
function readProbeAuthTokens(codexHome: string): ProbeAuthTokens | null {
	...
	const accountId =
		accountIdFromToken || asString(authAccess?.chatgpt_account_id) || asString(authId?.chatgpt_account_id)
...
function getAliasHome(alias: string): string {
...
export function getProbeModels(): string[] {
	const raw = (
		process.env.OPENCODE_ENHANCER_LIMITS_PROBE_MODELS ||
		process.env.OPENCODE_MULTI_AUTH_LIMITS_PROBE_MODELS ||
		''
	).trim()
	const fromEnv = raw
		.split(',')
