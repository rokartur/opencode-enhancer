	...
	if (isRecord(input.notifications)) {
		const notifications: NotificationSettings = {
			...
			...(typeof input.notifications.taskComplete === 'boolean'
				...
				: {}),
			...(typeof input.notifications.error === 'boolean' ? { error: input.notifications.error } : {}),
			...(typeof input.notifications.question === 'boolean' ? { question: input.notifications.question } : {}),
...
export function validateSettings(settings: Partial<RotationSettings>): SettingsValidationError[] {
	...
	if (settings.criticalThreshold !== undefined) {
		if (typeof settings.criticalThreshold !== 'number' || Number.isNaN(settings.criticalThreshold)) {
			errors.push({
	...
	if (settings.notifications) {
		const entries = Object.entries(settings.notifications) as Array<[keyof NotificationSettings, unknown]>
