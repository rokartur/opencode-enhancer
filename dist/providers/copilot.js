                ...
                if (premium) {
                    ...
                    if (premium.unlimited) {
                        ...
                        if (premiumsRemaining < 0) {
                            utilization = entitlement > 0 ? ((entitlement + Math.abs(premiumsRemaining)) / entitlement) * 100 : 100;
                        }
                    ...
                    windows.push({
                        utilization: isUnlimited ? 0 : (premium.remaining !== undefined
                            ? (premium.remaining < 0 ? 100 : Math.round(((premium.entitlement ?? 0) - premium.remaining) / (premium.entitlement || 1) * 100))
                            : Math.round(utilization)),
                        label: `premium${plan ? ` (${planLabel})` : ''}`,
                        resetsAt: parseQuotaResetDate(copilotData.quota_reset_date_utc || copilotData.quota_reset_date || copilotData.limited_user_reset_date),
                    });
                }
                else {
                    ...
                    if (anyUnlimited && totalEntitlement === 0) {
                        ...
                        const used = totalRemaining < 0 ? totalEntitlement + Math.abs(totalRemaining) : totalEntitlement - totalRemaining;
