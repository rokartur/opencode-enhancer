...
function movePathIfMissing(source, target) {
    if (!fs.existsSync(source) || fs.existsSync(target))
        return;
    fs.mkdirSync(path.dirname(target), { recursive: true, mode: 0o700 });
    fs.renameSync(source, target);
}
function migrateLegacyStoreLocation() {
    if (storeLocationChecked)
        return;
    ...
    if (!fs.existsSync(LEGACY_DEFAULT_STORE_DIR))
        return;
    for (const suffix of ['', '.bak', '.lkg']) {
        movePathIfMissing(path.join(LEGACY_DEFAULT_STORE_DIR, `${DEFAULT_STORE_FILE}${suffix}`), path.join(DEFAULT_STORE_DIR, `${DEFAULT_STORE_FILE}${suffix}`));
    }
}
...
function validateStore(data) {
    ...
    return {
        ...
        forcedAlias: typeof data.forcedAlias === 'string'
            ? data.forcedAlias
            : typeof force?.forcedAlias === 'string'
                ? force.forcedAlias
                : null,
        forcedUntil: typeof data.forcedUntil === 'number'
            ? data.forcedUntil
            : typeof force?.forcedUntil === 'number'
                ? force.forcedUntil
                : null,
        previousRotationStrategy: typeof data.previousRotationStrategy === 'string'
            ? data.previousRotationStrategy
            : typeof force?.previousRotationStrategy === 'string'
                ? force.previousRotationStrategy
                : null,
        forcedBy: typeof data.forcedBy === 'string'
            ? data.forcedBy
