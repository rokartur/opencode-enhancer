...
export function getMinRemaining(rateLimits) {
    const fiveHour = rateLimits?.fiveHour?.remaining;
    const weekly = rateLimits?.weekly?.remaining;
    const values = [fiveHour, weekly].filter((v) => typeof v === 'number');
    if (values.length === 0)
        return Infinity;
    return Math.min(...values);
}
export function selectBestAvailableAccount(excludeAlias, threshold) {
    ...
    const eligible = aliases.filter(alias => {
        ...
        if (excludeAlias && alias === excludeAlias)
            return false;
        if (acc.rateLimitedUntil && acc.rateLimitedUntil > now)
            return false;
    ...
    if (eligible.length === 0)
        return null;
    eligible.sort((a, b) => {
        const remainingA = getMinRemaining(store.accounts[a].rateLimits);
        const remainingB = getMinRemaining(store.accounts[b].rateLimits);
        if (remainingB !== remainingA)
            return remainingB - remainingA;
        return (store.accounts[a].usageCount || 0) - (store.accounts[b].usageCount || 0);
    });
    if (typeof threshold === 'number') {
        const best = store.accounts[eligible[0]];
        if (getMinRemaining(best.rateLimits) < threshold)
            return null;
    }
    return eligible[0];
}
...
export async function getNextAccount(config) {
        ...
        if (forcedAccount) {
            ...
            if (health.isHealthy) {
                ...
                if (token) {
                    ...
                    console.warn(`[enhancer] Force mode: ${forcedAlias} token unavailable; refusing fallback`);
                    return null;
                }
            }
            else {
    ...
    const tokenFailureCooldownMs = (() => {
        const raw = readEnv('OPENCODE_ENHANCER_TOKEN_FAILURE_COOLDOWN_MS', 'OPENCODE_MULTI_AUTH_TOKEN_FAILURE_COOLDOWN_MS');
...
export function markModelUnsupported(alias, cooldownMs, info) {
    ...
    console.warn(`[enhancer] Account ${alias} marked model-unsupported for ${cooldownMs / 1000}s${extra}`);
}
...
export function markWorkspaceDeactivated(alias, cooldownMs, info) {
    ...
    console.warn(`[enhancer] Account ${alias} marked workspace-deactivated for ${cooldownMs / 1000}s`);
}
